...
This version fixes event marker placement for current logs.

Important:
- If each log line has only one number, x-axis is logged sample index, not real environment step.
- Therefore, event position is approximated using traffic type distribution.
- For averaged plots, the event is shown as a small range, because different traffic types
  appear with different probabilities.

Example:
python plot_ch5_3x3_plain_logs_fixed_event.py \
  --root ./log \
  --load heavy \
  --methods DRL-OR MAPPO-OR \
  --event link_failure:10000 \
  --event traffic_change:10000 \
  --out ./figures_ch5_3x3/combined_3x3_fixed_event.png \
  --moving-window 100
"""
...
METRIC_LABELS = {
    ...
    # If your loss values are 0.14 meaning 14%, do NOT write "(%)" here.
    # If you multiply loss by 100 before plotting, then you can change it to "Packet loss ratio (%)".
...
def parse_log_file(path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    If each line has two or more numbers:
        first number is treated as x,
        last number is treated as y.

    If each line has only one number:
        x is the line index,
        y is the logged value.

    Current logs usually have one number per line, so x is sample index.
    """
...
def average_types(curves: List[Tuple[np.ndarray, np.ndarray]]) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    """
    Average curves across traffic types.

    Because current logs have x = sample index per traffic type,
    this function averages over a common sample-index grid.
    """
...
def draw_event_marker(
    ...
    event_step: float,
    type_id: Optional[int],
    type_dist: List[float],
    x_divisor: float,
):
    """
    Draw event marker for current logs.

    Current logs usually have one value per line, so x-axis is sample index.
    If event happens at global environment step = event_step,
    then its approximate position in type-specific log is:

        event_sample_index ≈ event_step * P(type)

    For averaged plot:
        different traffic types have different probabilities,
        so event is shown as a range from min(P(type)) to max(P(type)).
    """
    if not type_dist:
        return

    if type_id is not None:
        if type_id >= len(type_dist):
            return

        event_x = (event_step * type_dist[type_id]) / x_divisor
        ...
        event_x_min = (event_step * min(type_dist)) / x_divisor
        event_x_max = (event_step * max(type_dist)) / x_divisor
        event_x_mid = 0.5 * (event_x_min + event_x_max)
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=Path("./figures_ch5_3x3/combined_3x3_delay_throughput_loss_fixed_event.png"))
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=float,
        default=[0.2, 0.3, 0.3, 0.2],
        help="Traffic type distribution. Used to approximate event position for current one-value-per-line logs.",
    )
    ...
    if len(args.type_dist) != args.num_types:
        raise ValueError(
            f"--type-dist length must match --num-types. "
            f"Got len(type_dist)={len(args.type_dist)}, num_types={args.num_types}"
        )
    ...
    for type_id in type_ids:
        ...
        for r, metric in enumerate(metrics):
            for c, scenario in enumerate(args.scenarios):
                ...
                if r == len(metrics) - 1:
                    if args.x_divisor == 1000:
                        ax.set_xlabel("Logged samples (× $10^3$)")
                    else:
                        ax.set_xlabel("Logged samples")
        ...
        if type_id is None:
            ...
            out = args.out_dir / args.load / f"type{type_id}" / "combined_3x3_delay_throughput_loss_fixed_event.png"
